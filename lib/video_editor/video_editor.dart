...
class VideoEditingResponse {
  final File videoFile;
  final Size videoDimensions;
  File? cover;

  VideoEditingResponse(this.videoFile, this.videoDimensions, {this.cover});
}
...
class _VideoEditorState extends State<VideoEditor> {
  ...
  void _openCropScreen() => Get.to(CropScreen(controller: _controller));
  ...
  void _exportVideo() async {
    ...
    await _controller.exportVideo(
      ...
      onCompleted: (file) async {
        ...
        if (!mounted) return;
        await _controller.extractCover(onCompleted: (cover) {
          Get.back<VideoEditingResponse>(
              result: VideoEditingResponse(
            file,
            _controller.videoDimension,
            cover: cover,
          ));
        });
        ...
        // 2.seconds.delay().then((value) => setState(() => _exported = false));
      },
    );
  }
    ...
    return [
      AnimatedBuilder(
        ...
        builder: (_, __) {
          ...
          return Padding(
            padding: EdgeInsets.all(8), // Margin.horizontal(height / 4),
            child: Row(children: [
